...
class ApiResponse
{
    ...
    public static function onlyEntityModify($data = null, $permissions = null)
    {
        $snakeCaseData = self::convertKeysToSnakeCase($data);

        $response = [];
        $response['message'] = 'Request was successful';
        $response['data'] = $snakeCaseData;
        $response['errors'] = null;
        $response = json_decode(json_encode($response));
    ...
    private static function convertKeysToSnakeCase($data)
    {
        if ($data === null) {
            return null;
        }
        ...
        foreach ($data as $key => $value) {
            $snakeCaseData[Str::snake($key)] = is_array($value) ? self::convertKeysToSnakeCase($value) : $value;
        }
